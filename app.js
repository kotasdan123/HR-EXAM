/* =========================================================
   SECURE EXAM PORTAL
   app.js
   Browser-only prototype using localStorage/sessionStorage
   ========================================================= */
...
function initializeApplication() {
    ...
    /*
     * IMPORTANT:
     * Violation detection is NOT started here.
     * It is started ONLY when an exam is actively running.
     */
...
function setupEventListeners() {
    ...
    const violationContinue = $("#violation-continue");

    if (violationContinue) {
        violationContinue.addEventListener("click", () => {
...
function handleActionClick(event) {
    ...
    switch (action) {
        ...
        case "delete-exam":
            deleteExam(button.dataset.examId);
            break;

        case "edit-exam":
            openExamModal(button.dataset.examId);
            break;
...
function logout() {
    ...
    /*
     * Stop all exam-related monitoring BEFORE logout.
     */
    stopExamMonitoring();
...
function showLogin() {
    ...
    /*
     * Extra protection:
     * Login must NEVER have an active violation overlay.
     */
    stopExamMonitoring();
...
function showExaminerDashboard() {
    ...
    const examinerView = $("#examiner-view");

    if (examinerView) {
        examinerView.hidden = false;
        examinerView.classList.add("active");
    }
...
function renderAdminDashboard() {
    ...
    setText("admin-total-exams", totalExams);
    setText("admin-total-submissions", totalAttempts);
    setText("admin-completed", completed);
    setText("admin-terminated", terminated);

    const recentContainer = $("#recent-submissions");
    ...
    recentContainer.innerHTML = recent.map(attempt => `
        <div class="submission-row">
            <div>
                ...
                <small>${escapeHTML(attempt.username)}</small>
            </div>
                ...
                <div class="exam-admin-info">
                    ...
                    <div class="exam-meta">
                        ...
                        <span>
                            Examiner:
                            <strong>
                                ${escapeHTML(exam.examinerUsername)}
                            </strong>
                        </span>
...
function saveExamFromModal() {
    ...
    const maxViolations =
        Number($("#exam-max-violations")?.value) ||
        CONFIG.DEFAULT_MAX_VIOLATIONS;
    ...
    const status =
        $("#exam-status")?.value || "active";

    if (!title || !formUrl || !examinerUsername) {
        alert("Please complete all required fields.");
        return;
    }

    if (
        !formUrl.startsWith("https://docs.google.com/") &&
        !formUrl.startsWith("https://forms.google.com/")
    ) {
        alert(
    ...
    if (!id && db.exams.length >= CONFIG.MAX_EXAMS_PER_ADMIN) {
...
function renderExaminerDashboard() {
    ...
    const container = $("#examiner-exams");

    if (!container || !currentUser) return;

    const assignedExams = db.exams.filter(
        exam =>
            exam.examinerUsername.toLowerCase() ===
    ...
    container.innerHTML =
        assignedExams.map(exam => {
            ...
            const attempt =
                getLatestAttempt(exam.id);

            const available =
                isExamAvailable(exam);

            const completed =
                hasCompletedAttempt(exam.id);

            let buttonText = "Start Exam";
            let disabled = false;

            if (completed) {
                buttonText = "Already Taken";
                disabled = true;
            } else if (!available) {
...
function isExamAvailable(exam) {
    ...
    if (exam.availabilityStart) {
        ...
        const start =
            new Date(exam.availabilityStart);
...
function hasCompletedAttempt(examId) {
    ...
    return getUserAttempts(examId).some(
        attempt =>
...
function startExam(examId) {
    ...
    if (!isExamAvailable(exam)) {
        alert("This exam is currently unavailable.");
        return;
    }
...
function beginExam(exam) {
    ...
    /*
     * Clear any old monitoring state first.
     */
    stopExamMonitoring();
    ...
    examState = {
        ...
        antiCheat: Boolean(exam.antiCheat),

        maxViolations:
            Number(exam.maxViolations) ||
            CONFIG.DEFAULT_MAX_VIOLATIONS,

        timerEnabled:
            Boolean(exam.timerEnabled),

        durationSeconds:
            Number(exam.durationSeconds) ||
    ...
    loadGoogleForm(exam.formUrl);

    /*
     * VERY IMPORTANT:
     * Monitoring starts ONLY here.
     */
...
function startExamTimer() {
    ...
    if (!examState.timerEnabled) return;
 ...
 * IMPORTANT:
 *
 * These listeners MUST NOT be active on:
 * - Login page
 * - Admin dashboard
 * - Examiner dashboard
 * - Exam instructions
 * - Result page
 *
 * They are registered only while:
 *
 * examState.active === true
 *
 * AND
 *
 * examState.antiCheat === true
 */
...
function handleWindowBlur() {
    ...
    /*
     * Small grace period prevents false positives.
     */
    if (focusTimer) {
        clearTimeout(focusTimer);
    }
...
function registerViolation(reason) {
    ...
    /*
     * Prevent duplicate events fired almost simultaneously.
     */
    const now = Date.now();

    if (
        examState.lastViolationAt &&
        now - examState.lastViolationAt < 1000
    ) {
        return;
    }
...
function showViolationOverlay(reason) {
    ...
    /*
     * CRITICAL:
     * The violation modal can ONLY appear
     * during an active anti-cheat exam.
     */
    if (!examState?.active) return;
...
function syncViolationOverlay() {
    ...
    /*
     * If there is no active exam,
     * the violation overlay MUST be hidden.
     */
    if (
        !examState?.active ||
        !examState?.antiCheat
...
function finishExam(status, message) {
    ...
    /*
     * Make sure the violation overlay
 ...
 *
 * 1. Blocking a user from opening another browser window.
 * 2. Blocking another device.
 * 3. Detecting every possible DevTools action.
 * 4. Reading the actual Submit button inside a
 *    cross-origin Google Form.
 *
 * The application can detect common focus/tab/fullscreen
 * events and record violations while the examination is active.
 *
 * For production-grade security, use a backend server,
 * authenticated sessions, server-side exam state,
 * server-side submission validation and a controlled
 ...
 *
 * It ensures that if the page loads on:
 * - Login
 * - Admin
 * - Examiner dashboard
 *
 * the violation overlay remains hidden.
 */
syncViolationOverlay();
